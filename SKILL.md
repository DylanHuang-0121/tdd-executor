...
```bash
python -m tdd_toolkit pipeline --project <project-name>
```
...
```bash
python -m tdd_toolkit progress --project <project-name> --target <target-node>
```
...
```bash
python -m tdd_toolkit list --feature <feature-name>
```
...
1. **规划阶段**
   ...
   Agent 行动：
   - 理解需求
   - 运行：python -m tdd_toolkit progress --project <name> --target planning
   - 创建任务文档
   ```
...
2. **测试阶段**
   ...
   Agent 行动：
   - 编写测试代码
   - 运行：python -m tdd_toolkit progress --project <name> --target unit_test
   - 运行测试（预期失败）
   ```
...
3. **实现阶段**
   ...
   Agent 行动：
   - 编写实现代码
   - 运行：python -m tdd_toolkit progress --project <name> --target code_implementation
   - 运行测试（预期通过）
   ```
...
4. **验证阶段**
   ```
   Agent 行动：
   - 运行：python -m tdd_toolkit progress --project <name> --target compile
   - 运行：python -m tdd_toolkit progress --project <name> --target run_tests
   - 确保所有测试通过
   ```
...
5. **调试阶段**（如需要）
   ...
   Agent 行动：
   - 如果测试失败
   - 运行：python -m tdd_toolkit progress --project <name> --target debugging
   - 运行：python -m tdd_toolkit record 记录问题
   - 修复后重新测试
   ```
...
# 2. 创建 Pipeline
python -m tdd_toolkit pipeline --project webhook-approval

# 3. 规划阶段
python -m tdd_toolkit progress --project webhook-approval --target planning
# Agent 创建任务文档，规划功能点

# 4. 测试阶段
python -m tdd_toolkit progress --project webhook-approval --target unit_test
# Agent 编写测试代码
# 运行测试（预期失败）

# 5. 实现阶段
python -m tdd_toolkit progress --project webhook-approval --target code_implementation
# Agent 编写实现代码
# 运行测试（预期通过）

# 6. 验证阶段
python -m tdd_toolkit progress --project webhook-approval --target compile
python -m tdd_toolkit progress --project webhook-approval --target run_tests
# 确保所有测试通过
...
# 8. 调试并重新测试
python -m tdd_toolkit progress --project webhook-approval --target debugging
# Agent 分析问题，修复代码
# 重新运行测试
```
...
**解决**：
1. 检查当前状态：`python -m tdd_toolkit pipeline --project <name>`
2. 确保前一个节点的任务已完成
